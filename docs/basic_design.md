...
>
> **デプロイ構成に関する注記**：現時点で本アプリは実環境へのデプロイを行っておらず、`config/deploy.yml`・`Dockerfile` は `rails new` 生成時の Kamal 既定値をベースとしている。本書のハードウェア／ネットワーク構成図に示す TLS 終端（kamal-proxy / Let's Encrypt）・サーバIP・コンテナレジストリ・モデレーションAPI（FastAPI）の配置は「実デプロイ時に想定される構成」であり、設定ファイル上は未確定（`proxy`・`accessories` 等は既定でコメントアウト）。実際にデプロイする際に、対象サーバIP・ドメイン／TLS・レジストリ・FastAPI の稼働方法を確定し、`config/deploy.yml` に反映する。
...
> 注：以下の構成図は実デプロイ時の想定構成であり、現時点の `config/deploy.yml` は Kamal 既定値（サーバIP・レジストリはプレースホルダ、`proxy` の SSL/host・`accessories` は未設定）のままである。FastAPI モデレーションAPI も deploy.yml には未定義で、実デプロイ時に稼働方法（accessory 化・別ホスト・手動起動等）を確定する。詳細は冒頭の「デプロイ構成に関する注記」を参照。
